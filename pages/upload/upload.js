...
const md5 = (function() {
  ...
  return function(arrayBuffer) {
    ...
    for (let i = 0; i < len; i += 4) {
      words[i >> 2] = uint8[i] | (uint8[i + 1] << 8) | (uint8[i + 2] << 16) | (uint8[i + 3] << 24);
    }
    return binl2hex(binlMD5(words, len * 8));
  };
})();
  ...
  async uploadImages() {
    ...
    for (const path of this.data.images) {
      try {
        let md5 = '';
        try {
          md5 = await this.getFileMD5(path);
        } catch (e) {
          console.warn('计算MD5失败', e);
        }
        ...
        const addRes = await new Promise((resolve, reject) => {
          wx.cloud.callFunction({
            name: 'addImage',
            data: { fileID: uploadRes.fileID, md5 },
            success: resolve,
            fail: reject
          });
        });
